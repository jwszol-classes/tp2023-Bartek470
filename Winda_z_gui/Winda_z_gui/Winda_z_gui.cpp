...
void arrive(std::vector <Human>& qpassengers)
{
    ...
    for (int x = 0; x <qsize; x++)
    {
        if (qpassengers[x].get_isinside() == 1 && qpassengers[x].get_end() == elevator.get_curentposition())
        {
            ...
            //qpassengers.erase(qpassengers.begin() + x);
            eraser.push_back(x);
        }
    }
...
void OnPaint(HDC hdc)
{
   ...
   // Pen pen6(Color(255, 255, 255, 0), 2.0); //bialy - nie uzywany
    //piętra
    ...
    case WM_TIMER: //do przepisania
        ...
        case Timer1:
            if (qpassengers.size() != 0)
            {
                ...
                /*if (!qpassengers[0].get_isinside()) { // sprawdza czy w windzie ktos jest / ma byc 
                    if (qpassengers[0].start_floor != elevator.curentposition) { // jezeli pasazer jest na innym pietrze
                        if (i != (qpassengers[0].start_floor - elevator.curentposition) * 120) { //wyznacza ilosc pikseli o jakie ma sie przemiescic do pietra startowego
                            i = i + (qpassengers[0].start_floor - elevator.curentposition) * 2;
                            offset = offset + (qpassengers[0].start_floor - elevator.curentposition) * 2;
                        }
                        else {
                            elevator.curentposition = qpassengers[0].start_floor;
                        }
                    }
                    else {
                        qpassengers[0].is_inside = true;
                        i = 0;
                    }
                }
                else {
                    if (qpassengers[0].end_floor != elevator.curentposition) {
                        if (i != (qpassengers[0].end_floor - elevator.curentposition) * 120) {
                            i = i + (qpassengers[0].end_floor - elevator.curentposition) * 2;
                            offset = offset + (qpassengers[0].end_floor - elevator.curentposition) * 2;
                        }
                        else {
                            elevator.curentposition = qpassengers[0].end_floor;
                        }
                    }
                    else {
                        i = 0;
                        Usuniecie_pierwszego(qpassengers);
                        if (qpassengers.empty()) {
